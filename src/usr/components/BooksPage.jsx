...
import BooksTutorialModal from "./BooksTutorialModal"
...
export default function BooksPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await getDataFromSource({
        dTable: "f_fonti",
        dQueryString:
          "fields=id,titolo_breve,titolo_esteso,anno_edizione,genere,lingua,formato,link_digitalizzazione,luogo_di_conservazione,biblioteca,segnatura,note_paratesti,note_sul_privilegio,associazione_luogo_di_stampa.nome_localita,personalita_coinvolte.tipo_di_rapporto,personalita_coinvolte.note_sul_rapporto_,personalita_coinvolte.f_persone_id.nome_e_cognome&limit=-1",
      })
      setData(result || [])
    }
  ...
  const genres = [...new Set(data.map((item) => item.genere))]
    .filter(Boolean)
    .sort()

  const languages = [...new Set(data.map((item) => item.lingua))]
    .filter(Boolean)
    .sort()

  const formats = [...new Set(data.map((item) => item.formato))]
    .filter(Boolean)
    .sort()

  const places = [
    ...new Set(data.map((item) => item.associazione_luogo_di_stampa?.nome_localita)),
  ]
    .filter(Boolean)
    .sort()
  ...
  const filteredData = data.filter((item) => {
    const matchGenre =
      selectedGenres.length === 0 || selectedGenres.includes(item.genere)
    const matchLanguage =
      selectedLanguages.length === 0 || selectedLanguages.includes(item.lingua)
    const matchFormat =
      selectedFormats.length === 0 || selectedFormats.includes(item.formato)
    const matchPlace =
      selectedPlaces.length === 0 ||
      selectedPlaces.includes(item.associazione_luogo_di_stampa?.nome_localita)
    ...
    const matchYear =
      !item.anno_edizione ||
      (item.anno_edizione >= yearRange[0] &&
        item.anno_edizione <= yearRange[1])
    ...
    return (
      <div className="d-flex justify-content-center align-items-center mt-4 flex-wrap">
                ...
                {number === "..." ? (
                  ...
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor:
                        number === currentPage ? colors.primary : "transparent",
                      borderColor: colors.secondary,
                    }}
                  >
                    {number}
        ...
        >
          ...
          {Math.min(indexOfLastItem, filteredData.length)} of {filteredData.length} items
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      >
        ...
        >
          Dataset is currently under review; some data may be inaccurate or partially
          incorrect.
        </span>
      </div>
                    ...
                    <Stack gap={1}>
                      {genres.map((genre) => (
                        <Form.Check
                          ...
                          onChange={() =>
                            toggleFilter(selectedGenres, setSelectedGenres, genre)
                          }
                        />
                      ))}
                    </Stack>
                    ...
                    <Stack gap={1}>
                      {places.map((place) => (
                        <Form.Check
                          ...
                          onChange={() =>
                            toggleFilter(selectedPlaces, setSelectedPlaces, place)
                          }
                        />
                      ))}
                    </Stack>
                          ...
                          {item.personalita_coinvolte?.length > 0 && (
                            <div className="mt-4">
                              ...
                              <div className="row g-3">
                                {item.personalita_coinvolte.map((person, index) => (
                                  <div
                                    key={person.id || `${item.id}-person-${index}`}
                                    className="col-md-6"
                                  >
                                    <div
                                      style={{
